...
memoria condivisa (si supponga che tale zona di memoria sia costituita da uno struct contenente un int, un float
e un char). Si supponga che normalmente il processo produttore scrive il valore intero e il valore reale. Il
processo consumatore legge questi numeri e li stampa a video. Quando il processo produttore desidera
terminare scrive il campo char inserendo il valore ‘f’. Se il processo consumatore legge il valore ‘f’ nel campo
char, allora esso termina. Si utilizzino dei semafori per la gestione della regione critica.
*/
...
int main(void)
{
  ...
  while(Running)
  {
    ...
    {
      printf("E' stato inserito %d come intero e %.2f come float.\n", P->a, P->b);
    }
