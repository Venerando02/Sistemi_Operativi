...
i parametri del main (argc e argv) per passare da tastiera il numero di figli da creare. Supporre che ciascun
processo figlio stampa a video una frase del tipo “Sono il processo figlio con PID = (valore del pid)” e termina.
E’ richiesta la gestione della terminazione dei processi figli tramite l’uso dei segnali, definendo una opportuna
funzione di SignalHandler che eviti che i figli diventino zombie.
*/
...
void SignalHandlerPadre(int segnale)
{
  ...
  while((pid = wait(&StatVal)) > 0)
  {
    ...
    {
      printf("PADRE: figlio PID = %d terminato correttamente.\n", pid);
