...
int main(void)
{
  ...
  if(ShmID == -1)
  {
    fprintf(stderr, "Impossibile settare la memoria condivisa.\n");
    exit(EXIT_FAILURE);
  }
  ...
  printf("Shared memory definita correttamente.\n");
  
  while(Running)
  {
    if(SEM_P(ReaderID) == -1) exit(EXIT_FAILURE);
    printf("Sono stati inseriti i valori %d e %.2f.\n", P->a, P->b);
    printf("Calcolo il prodotto.\n");
  ...
  printf("Shared memory eliminata correttamente.\n");
