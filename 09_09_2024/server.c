...
void * ConnectionHandler(void * clientfd)
{
  ...
  char* StringaOutput = NULL;
  int i, j;
  
  while(read(descriptor, StringaInput, STR_LEN) > 0)
  {
    j = 0;
    
    StringaOutput = (char*)malloc(sizeof(char)*STR_LEN);
    
    for(i = (strlen(StringaInput) - 1); i>=0; i--)
    {
      StringaOutput[j++] = StringaInput[i];
    }    
    write(descriptor, StringaOutput, STR_LEN);
  }
